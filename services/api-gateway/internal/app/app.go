...
import (
	"monorepo/libs/config"
	"monorepo/libs/database"
	"monorepo/libs/httputil/middleware"
	"monorepo/libs/logger"
	"monorepo/services/api-gateway/internal/features/auth"
	"monorepo/services/api-gateway/internal/features/user"
	"monorepo/services/api-gateway/internal/repository"
...
func (a *App) setupRouter() *gin.Engine {
	...
	userRepo := repository.NewUserRepository(a.DB.GetDB())

	// Register all features - just add one line per new feature!
	features := []Feature{
		auth.NewModule(userRepo, a.Logger, a.Config.JWT.Secret),
		user.NewModule(userRepo, a.Logger, a.Config.JWT.Secret),
	}
