...
import (
	...
	svcMiddleware "monorepo/services/api-gateway/internal/middleware"
)
...
func (h *UserHandler) GetProfile(c *gin.Context) {
	...
	userID, exists := c.Get(svcMiddleware.UserIDKey)
	if !exists {
		response.NewResponse(c, http.StatusUnauthorized, nil, errors.GetErrorMessage(errors.SomethingWentWrong, lang), nil)
		return
	}
...
func (h *UserHandler) UpdateProfile(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		response.NewResponse(c, http.StatusBadRequest, nil, errors.GetErrorMessage(errors.ValidationFailed, lang), map[string][]string{
			"body": {err.Error()},
		})
		return
	}
	...
	// Validate request
	if errs := req.Validate(lang); len(errs) > 0 {
		response.NewResponse(c, http.StatusBadRequest, nil, errors.GetErrorMessage(errors.ValidationFailed, lang), errs)
		return
	}
